...
SELECT 
    AVG(item_count) AS avg_items_per_paid_order
FROM (
    SELECT 
        oi.order_id,
        SUM(oi.quantity) AS item_count
    FROM order_items oi
    WHERE oi.order_id IN (SELECT order_id FROM orders WHERE status = 'paid')
...
SELECT 
    ROUND(
        100.0 * SUM(CASE WHEN payment_method = 'app' THEN 1 ELSE 0 END) / COUNT(*),
        2
